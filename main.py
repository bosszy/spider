...
def get_images_from_baidu(keyword, page_num, save_dir):
    ...
    for pn in range(0, 30 * page_num, 30):
        ...
        for image_url in image_url_list:
            ...
            if keyword == '猫':
                with open(os.path.join(save_dir, f'cat-{n:d}.jpg'), 'wb') as fp:
                    fp.write(image_data)
            elif keyword == '狗':
                with open(os.path.join(save_dir, f'dog-{n:d}.jpg'), 'wb') as fp:
                    fp.write(image_data)
            n = n + 1


if __name__ == '__main__':
    t1 = Thread(target=get_images_from_baidu('猫', 10, '猫'))
    t1.start()
    t2 = Thread(target=get_images_from_baidu('狗', 10, '狗'))
    t2.start()
    t1.join()
    t2.join()
